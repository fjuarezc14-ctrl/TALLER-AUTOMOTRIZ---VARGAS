...
const ALERTAS_CFG = [
  { key: 'km_ultimo_aceite',       label: 'Aceite',       umbral: 8000,  icon: '🛢️' },
  { key: 'km_ultimo_filtros',      label: 'Filtros',      umbral: 15000, icon: '🔧' },
  { key: 'km_ultimo_frenos',       label: 'Frenos',       umbral: 30000, icon: '🛑' },
  { key: 'km_ultimo_bujias',       label: 'Bujías',       umbral: 40000, icon: '⚡' },
  { key: 'km_ultimo_refrigerante', label: 'Refrigerante', umbral: 40000, icon: '💧' },
  { key: 'km_ultimo_distribucion', label: 'Distribución', umbral: 80000, icon: '⚙️' },
];
  ...
  ficha.innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
      ...
      <div style="display:flex;gap:8px;">
        ...
        <a href="https://wa.me/51${(cliente.telefono||'').replace(/\D/g,'')}?text=${encodeURIComponent(`Estimado(a) ${cliente.nombre.split(' ')[0]}, le saludamos del Taller Automotriz Vargas.`)}" target="_blank" class="btn-success" style="font-size:12px;text-decoration:none;display:flex;align-items:center;gap:5px;">
  ...
  const templates = [
    {
      ...
      texto: `Estimado/a ${nombre}, le saludamos del *Taller Automotriz Vargas*. Su vehículo *${modelo} (${placa})* se encuentra listo para su entrega. Detalle: *${monto}*. Puede pasar a recogerlo en horario de atención. ¡Gracias por su preferencia!`
    },
    {
      icon: '🔔',
      label: 'Recordatorio Mantenimiento',
      texto: `Hola ${nombre}, le saludamos del *Taller Automotriz Vargas*. Notamos que su vehículo *${placa}* está próximo a requerir: *${svcPend}*. ¿Le gustaría agendar una cita esta semana? Contáctenos y le brindamos el mejor servicio. ¡Le esperamos!`
    },
    {
      icon: '⭐',
